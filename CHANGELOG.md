...
## v1.0.47 — 2026-09-15 · إصلاح شامل للأذان وبطاقات صفحة القرآن

### 🕌 الأذان — إصلاحات وتحسينات
- **خلل رسالة كتم الأذان**: كانت الرسالة تظهر كود HTML حرفياً
  (`<i class="fas fa-volume-xmark"></i>`) لأن `showToast` كان يستخدم
  `textContent`. الآن يستخدم `innerHTML` مع مُنقّي HTML (`sanitizeToastHtml`)
  يسمح فقط بوسوم آمنة (`<i>`, `<b>`, `<span>`, `<br>`, `<strong>`, `<em>`)
  ويزيل أي سمات خطرة (`onclick`, `<script>`, `<img onerror>`…) — الرسائل
  تظهر بأيقوناتها الصحيحة مع حماية كاملة من الحقن.
- **وضع «أول تكبيرتين فقط»**: إصلاح استدعاء `onEnded` المزدوج (كان يُستدعى
  حتى عند فشل التحميل) — الآن يُستدعى مرة واحدة فقط عند اكتمال التشغيل أو
  القطع المبرمج، فلا يتعارض مع منطق المتابعة في `playFullAdhan`.
- **معاينة «أول تكبيرتين»**: تطبيق نفس الحد الأدنى (5 ثوانٍ) المستخدم في
  الأذان الفعلي حتى تطابق المعاينة ما سيُسمع في وقت الصلاة تماماً.
- التبديل بين «الأذان كامل» و«أول تكبيرتين فقط» يعمل فعلياً: الحفظ في
  `Storage` ثم إعادة الرسم ومعاينة فورية عند اختيار وضع التكبيرتين.

### 🎚️ مفاتيح التفعيل — وضوح الحالة
- **مفعّل = أخضر واضح** (`#1E9E6A` مع هالة خفيفة) في كل الثيمات — ألغيت
  ألوان الثيمات الذهبية/البنية التي كانت تجعل المفتاح المفعّل يبدو باهتاً.
- **معطّل = رمادي واضح** (`#C7CDD4` فاتح / `#4A5560` في الوضع الداكن).
- ينطبق على: كتم الأذان، إشعارات الصلاة، التذكير قبل الأذان، وكل المفاتيح.

### 📖 صفحة القرآن الرئيسية — بطاقتا «المحفوظات» و«آخر آية»
- إصلاح تضارب `margin`/`width: calc(100% - ...)` داخل الحاوية المرنة الذي
  كان يجعل البطاقتين تخرجان عن المسار أو تتكدسان.
- أيقونة 34px، عنوان 13px، نص فرعي 11px مع قصّ تلقائي للنصوص الطويلة
  (ellipsis)، وارتفاع أدنى 64px مناسب للمس.
- قاعدة للشاشات الصغيرة جداً (<380px): أحجام أصغر قليلاً تبقى مقروءة.
