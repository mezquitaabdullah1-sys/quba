...
const LocationService = {
  ...
  async getCurrent(options = {}) {
    ...
    if (!forceRefresh) {
        // v39: una ubicación cacheada de ANTES de esta versión no tiene
        ...
        if (typeof cached.elevation !== 'number') {
  ...
  // v39: ahora async — también resuelve la elevación (Open-Meteo) antes de
  // devolver, para que el PRIMER cálculo de horarios tras elegir una ciudad
  // ya salga con Shuruq/Maghrib/Isha corregidos (si no, quedaría cacheado
  // hasta 14 días sin la corrección). El único llamador actual (selector de
  // ciudad en Perfil) no dependía del valor de retorno síncrono, así que
  // añadir `await` ahí es seguro — solo se retrasa el re-render lo que tarda
  // la consulta de elevación (normalmente bajo un segundo).
  async setManual(lat, lon, city, country) {
    ...
    const coords = {
      ...
      elevation, // v39: metros s.n.m., o null si no se pudo obtener
      manual: true,
    };
